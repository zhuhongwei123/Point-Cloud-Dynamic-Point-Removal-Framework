...
Removerter::Removerter() {
    ...
    save_remove_map_img = save_pcd_directory_ + "/remove/map_rimg/";
    fsmkdir(save_remove_map_img);
    save_remove_scan_img = save_pcd_directory_ + "/remove/scan_rimg/";
    fsmkdir(save_remove_scan_img);
    save_remove_diff_img = save_pcd_directory_ + "/remove/diff_rimg/";
    fsmkdir(save_remove_scan_img);
    save_revert_map_img = save_pcd_directory_ + "/revert/map_rimg/";
    fsmkdir(save_revert_map_img);
    save_revert_scan_img = save_pcd_directory_ + "/revert/scan_rimg/";
    fsmkdir(save_revert_scan_img);
    dyna_file_name_final = save_pcd_directory_ + "/final/";
    fsmkdir(dyna_file_name_final);
    static_file_name_final = save_pcd_directory_ + "/final/";;
    fsmkdir(static_file_name_final);
...
void Removerter::allocateMemory() {
    ...
    map_global_curr_dynamic_all_res.reset(new pcl::PointCloud<PointType>());
    ...
    map_global_static.reset(new pcl::PointCloud<PointType>());
    //
...
//        }
    ...
    if (use_keyframe_gap_) {
        ROS_INFO_STREAM("\033[1;32m Total " << sequence_valid_scan_paths_.size()
                                            << " nodes are used from the index range [" << start_idx_ << ", "
                                            << end_idx_ << "]"
...
// for target range of scan idx 
{
    ...
    for (auto &_scan_path: sequence_valid_scan_paths_) {
        ...
        pcl::PointCloud<PointType>::Ptr points(
                new pcl::PointCloud<PointType>); // pcl::PointCloud Ptr is a shared ptr so this points will be automatically destroyed after this function block (because no others ref it).
        // bin 文件是为了kitty数据集
...
//        scans_.emplace_back(points);
    ...
    cout << "读取到的帧的数量是&&&&&&&&&&&&&&&&&&" << scans_.size() << endl;
    cout << endl;
} // readValidScans
...
std::pair<cv::Mat, cv::Mat> Removerter::map2RangeImg(const pcl::PointCloud<PointType>::Ptr &_scan,
    ...
    cv::Mat rimg_color_MAP = cv::Mat(kNumRimgRow, kNumRimgCol, CV_32FC1,
                                     cv::Scalar::all(kFlagNoPOINT)); // float matrix, save range value
    cv::Mat rimg_ptidx = cv::Mat(kNumRimgRow, kNumRimgCol, CV_32SC1,
...
//            rimg_color_MAP.at<cv::Vec3b>(pixel_idx_row,pixel_idx_col) = cv::Vec3b(254-int(this_point.x *2 ),254 -int(fabs(this_point.y)/0.5),254-int(fabs(this_point.z + 1.0f)/0.05));
            rimg_ptidx.at<int>(pixel_idx_row, pixel_idx_col) = pt_idx;

//            cv::imwrite("/home/zhuhongwei/Removert_ws/src/removert/zhuhongweiTest/map_rimg" + std::to_string(idx_scan) + ".jpg",rimg_color_MAP);
...
cv::Mat Removerter::scan2RangeImg(const pcl::PointCloud<PointType>::Ptr &_scan,
    ...
    cv::Mat rimg_color_Scans = cv::Mat(kNumRimgRow, kNumRimgCol, CV_32FC1,
                                       cv::Scalar::all(kFlagNoPOINT)); // float matrix

    // @ points to range img 
    int num_points = _scan->points.size();
    cout << "scan 里面有" << num_points << "个点云" << endl;
#pragma omp parallel for num_threads(kNumOmpCores)
    for (int pt_idx = 0; pt_idx < num_points; ++pt_idx) {
        PointType this_point = _scan->points[pt_idx];
        SphericalPoint sph_point = cart2sph(this_point);
...
//        cout<< "pt_idx"<<pt_idx<<endl;
        ...
        // @ P.S. some explicit mutexing directive makes the code even slower ref: https://stackoverflow.com/questions/2396430/how-to-use-lock-in-openmp
        if (curr_range < rimg.at<float>(pixel_idx_row, pixel_idx_col)) {
//            rimg_color_Scans.at<cv::Vec3b>(pixel_idx_row,pixel_idx_col) = cv::Vec3b(254-int(this_point.x *2 ),254 -int(fabs(this_point.y)/0.5),254-int(fabs(this_point.z + 1.0f)/0.05));
//            cv::imwrite("/home/zhuhongwei/Removert_ws/src/removert/zhuhongweiTest/scan_rimg" + std::to_string(number)+ ".jpg",rimg_color_Scans);
            rimg.at<float>(pixel_idx_row, pixel_idx_col) = curr_range;
        }
    }
...
Removerter::scans2batch(pcl::PointCloud<PointType>::Ptr ScanFirst, pcl::PointCloud<PointType>::Ptr ScanSecond,
                        Eigen::Matrix<double, 4, 4> PoseFirst, Eigen::Matrix<double, 4, 4> PoseSecond) {

    Eigen::Matrix<double, 4, 4> Transform_World2ScanFirst = PoseFirst.inverse();
    Eigen::Matrix<double, 4, 4> Transform_ScanSecond2ScanFirst = Transform_World2ScanFirst * PoseSecond;
    pcl::PointCloud<PointType>::Ptr PointCloud2ScanFist(new pcl::PointCloud<PointType>());
    pcl::transformPointCloud(*ScanSecond, *PointCloud2ScanFist, Transform_ScanSecond2ScanFirst);
    *ScanFirst += *PointCloud2ScanFist;
    return ScanFirst;
};
...
void Removerter::mergeScansWithinGlobalCoord(
    ...
    for (std::size_t scan_idx = 0; scan_idx < _scans.size(); scan_idx++) {   /// scans 是雷达坐标系下的点，_scans_poses是世界坐标系下的
        ...
        pcl::transformPointCloud(*ii_scan, *scan_global_coord,
                                 kSE3MatExtrinsicLiDARtoPoseBase); /// 将点云从雷达坐标系变换到位姿统一（kitty就是以相机的坐标为基准）的坐标系下
        pcl::transformPointCloud(*scan_global_coord, *scan_global_coord, ii_pose); // 变换到第一针坐标系下

        // merge the scan into the global map，一直将每一帧全局地图累加到一起
...
void
Removerter::octreeDownsampling(const pcl::PointCloud<PointType>::Ptr &_src, pcl::PointCloud<PointType>::Ptr &_to_save) {
...
void Removerter::makeGlobalMap(void) {
    ...
    map_global_orig_->clear();
    map_global_curr_->clear();
    ROS_INFO_STREAM("\033[1;32m map_global_orig_ and map_global_curr_ have been clean. \033[0m");
    map_global_curr_dynamic_all_res->clear();
/// 从scans_中都出来的都是以当前雷达坐标系下的点云，scan_poses_世界坐标系下的位姿（不一定是雷达坐标系下），这里转换到世界坐标系，并将点云保存在map_global_orig_，map_global_orig_这是一个大的指针，保存着所有的地图
...
//    ROS_INFO_STREAM("\033[1;32m Map pointcloud (having redundant points) have: " << map_global_orig_->points.size() << " points.\033[0m");
    ...
    if (kFlagSaveMapPointcloud) {
        ...
        /// 将map_global_curr_变换到base_node_idx_个索引的雷达坐标系下，比如base_node_idx == 0，意味着第一证是单位矩阵
        transformGlobalMapToLocal(map_global_curr_, base_node_idx, map_local_curr);///变到第一针的雷达坐标系下
        /// 如果第一针的初始位子是单位矩阵的话，这两个图应该是一样的。
        ...
        ROS_INFO_STREAM(
                "\033[1;32m The original pointcloud is saved (local coord): " << static_local_file_name << "\033[0m");
    }
...
void
Removerter::parseStaticMapPointcloudUsingPtIdx(pcl::PointCloud<PointType>::Ptr input, std::vector<int> &_point_indexes,
...
void
Removerter::parseDynamicMapPointcloudUsingPtIdx(pcl::PointCloud<PointType>::Ptr input, std::vector<int> &_point_indexes,
                                                pcl::PointCloud<PointType>::Ptr output) {
    // extractor
    pcl::ExtractIndices<PointType> extractor;
    boost::shared_ptr<std::vector<int>> index_ptr = boost::make_shared<std::vector<int>>(_point_indexes);
...
void Removerter::saveFinalStaticAndDynamicPointCloudGlobal() {

    pcl::io::savePCDFileBinary(dyna_file_name_final + "/finaldynamic.pcd", *map_global_curr_dynamic_);
    ROS_INFO_STREAM("\033[1;32m -- a pointcloud is saved: " << dyna_file_name_final + "/finaldynamic.pcd" << "\033[0m");

    // static

    pcl::io::savePCDFileBinary(static_file_name_final + "/finalStatic.pcd", *map_global_curr_);
    ROS_INFO_STREAM(
            "\033[1;32m -- a pointcloud is saved: " << static_file_name_final + "/finalStatic.pcd" << "\033[0m");
...
std::vector<int> Removerter::calcDescrepancyAndParseDynamicPointIdx
    ...
    int num_dyna_points{
            0}; // TODO: tracking the number of dynamic-assigned points and decide when to stop removing (currently just fixed iteration e.g., [2.5, 2.0, 1.5])
    ...
    for (int row_idx = 0; row_idx < _diff_rimg.rows; row_idx++) {
        for (int col_idx = 0; col_idx < _diff_rimg.cols; col_idx++) {
            float this_diff = _diff_rimg.at<float>(row_idx, col_idx); // 距离只差（地图与当前帧）
            float this_range = _scan_rimg.at<float>(row_idx, col_idx); // 当前帧的距离
            /// 自适应系数，后面可以调整的
            ...
            /// 有效差异的上限
            if (this_diff <
                kValidDiffUpperBound // exclude no-point pixels either on scan img or map img (100 is roughly 100 meter)
...
void Removerter::RemoveFormMatMAPSavePNG(cv::Mat rimg_, std::pair<float, float> kRangeColorAxis_, int idx_scan) {
    cv::Mat Map_rimg_color = convertColorMappedImg(rimg_, kRangeColorAxis_);
    cv::imwrite(save_remove_map_img + std::to_string(idx_scan) + ".jpg", Map_rimg_color);
}

void Removerter::RemoveForDiffMAPSavePNG(cv::Mat rimg_, std::pair<float, float> kRangeColorAxis_, int idx_scan) {
    cv::Mat Map_rimg_color = convertColorMappedImg(rimg_, kRangeColorAxis_);
    cv::imwrite(save_remove_diff_img + std::to_string(idx_scan) + ".jpg", Map_rimg_color);
}

void Removerter::RemoveFormMatScansSavePNG(cv::Mat rimg_, std::pair<float, float> kRangeColorAxis_, int idx_scan) {
    cv::Mat scan_rimg_color = convertColorMappedImg(rimg_, kRangeColorAxis_);
    cv::imwrite(save_remove_scan_img + std::to_string(idx_scan) + ".jpg", scan_rimg_color);
}

void Removerter::RevertFormMatMAPSavePNG(cv::Mat rimg_, std::pair<float, float> kRangeColorAxis_, int idx_scan) {
    cv::Mat Map_rimg_color = convertColorMappedImg(rimg_, kRangeColorAxis_);
    cv::imwrite(save_revert_map_img + std::to_string(idx_scan) + ".jpg", Map_rimg_color);
}

void Removerter::RevertFormMatScansSavePNG(cv::Mat rimg_, std::pair<float, float> kRangeColorAxis_, int idx_scan) {
    cv::Mat scan_rimg_color = convertColorMappedImg(rimg_, kRangeColorAxis_);
    cv::imwrite(save_revert_scan_img + std::to_string(idx_scan) + ".jpg", scan_rimg_color);
}

std::vector<int>
Removerter::calcDescrepancyAndParseDynamicPointIdxForEachScanForRevert(std::pair<int, int> _rimg_shape) {
    std::vector<int> dynamic_point_indexes;
    ...
    for (std::size_t idx_scan = 0; idx_scan < scans_.size(); ++idx_scan) {
        ...
        if (idx_scan % 100 == 0) {
            ROS_INFO_STREAM("\033[1;32m -- ... have been solved" << (idx_scan * iter) << " number" "\033[0m");
            iter = iter + 1;
        }
        // curr scan
        pcl::PointCloud<PointType>::Ptr _scan = scans_.at(idx_scan);

        // scan's pointcloud to range img ，生成深度图
        cv::Mat scan_rimg = scan2RangeImg(_scan, kFOV, _rimg_shape, idx_scan); // openMP inside
        RevertFormMatScansSavePNG(scan_rimg, kRangeColorAxis, idx_scan);
        ...
        /// 把地图转换成深度图片，map_global_curr_dynamic_是每次remove后从静态地图生成的动态地图，然后加入到一个向量里，然后经过去重生成的，由于revert是在remove后，所以
        /// map_global_curr_dynamic_ 代表里所有remove后生成的动态点云
        auto[map_rimg, map_rimg_ptidx] = map2RangeImg(map_global_curr_dynamic_all_res, kFOV, _rimg_shape,
                                                      idx_scan); // the most time comsuming part 2 -> so openMP applied inside
        RevertFormMatMAPSavePNG(map_rimg, kRangeColorAxis, idx_scan);
        ...
        /// 其中有个阈值的判断，但是非常的松，只要两个地图与当前帧的距离有差距，就会认为是动态点
        std::vector<int> this_scan_dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdx(scan_rimg, diff_rimg,
                                                                                                  map_rimg_ptidx);
        ...
        // visualization
//        pubRangeImg(scan_rimg, scan_rimg_msg_, Revert_scan_rimg_msg_publisher_, kRangeColorAxis);
//        pubRangeImg(map_rimg, map_rimg_msg_, Revert_map_rimg_msg_publisher_, kRangeColorAxis);
//        pubRangeImg(diff_rimg, diff_rimg_msg_, Revert_diff_rimg_msg_publisher_, kRangeColorAxisForDiff);

        std::pair<float, float> kRangeColorAxisForPtIdx{0.0, float(map_global_curr_->points.size())};
//        pubRangeImg(map_rimg_ptidx, map_rimg_ptidx_msg_, Revert_map_rimg_ptidx_msg_publisher_, kRangeColorAxisForPtIdx);
//
...
std::vector<int> Removerter::calcDescrepancyAndParseDynamicPointIdxForEachScan(std::pair<int, int> _rimg_shape) {
    ...
    for (std::size_t idx_scan = 0; idx_scan < scans_.size(); ++idx_scan) {
        ...
        pcl::PointCloud<PointType>::Ptr _scan = scans_.at(idx_scan);
        ROS_INFO_STREAM("\033[1;32m -- ... have been solved" << (idx_scan) << " number" "\033[0m");
        // scan's pointcloud to range img ，生成深度图，_scan是在激光雷达坐标系下的
        cv::Mat scan_rimg = scan2RangeImg(_scan, kFOV, _rimg_shape, idx_scan); // openMP inside
        RemoveFormMatScansSavePNG(scan_rimg, kRangeColorAxis, idx_scan);
        // map's pointcloud to range img
        if (kUseSubsetMapCloud) {
            takeGlobalMapSubsetWithinBall(idx_scan);
            transformGlobalMapSubsetToLocal(idx_scan); // the most time comsuming part 1
        ...
        auto[map_rimg, map_rimg_ptidx] = map2RangeImg(map_local_curr_, kFOV, _rimg_shape,
                                                      idx_scan); // the most time comsuming part 2 -> so openMP applied inside
        RemoveFormMatMAPSavePNG(map_rimg, kRangeColorAxis, idx_scan);
        ...
        /// 计算两个矩阵的值的绝对值
        cv::absdiff(scan_rimg, map_rimg, diff_rimg);
        RemoveForDiffMAPSavePNG(diff_rimg, kRangeColorAxis, idx_scan);
        // parse dynamic points' indexes: rule: If a pixel value of diff_rimg is larger, scan is the further - means that pixel of submap is likely dynamic.
        /// 其中有个阈值的判断，但是非常的松，只要两个地图与当前帧的距离有差距，就会认为是动态点
        std::vector<int> this_scan_dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdx(scan_rimg, diff_rimg,
                                                                                                  map_rimg_ptidx);
        /// 每一帧的动态点都加入到这个向量里，这样往里面叉数据，肯定会有动态点云的重复问题，因此，下文会对于正好向量进行set去重
        dynamic_point_indexes.insert(dynamic_point_indexes.end(), this_scan_dynamic_point_indexes.begin(),
                                     this_scan_dynamic_point_indexes.end());
...
std::vector<int> Removerter::calcDescrepancyAndParseDynamicPointIdxForBatchScan(std::pair<int, int> _rimg_shape) {
    std::vector<int> dynamic_point_indexes;
    int size_scans = scans_.size();

    for (std::size_t idx_scan = 0; idx_scan < size_scans - 1; ++idx_scan) {

        pcl::PointCloud<PointType>::Ptr ScanFirst = scans_.at(idx_scan);
        pcl::PointCloud<PointType>::Ptr ScanSecond = scans_.at(idx_scan + 1);
        Eigen::Matrix<double, 4, 4> PoseFirst = scan_poses_.at(idx_scan);
        Eigen::Matrix<double, 4, 4> PoseSecond = scan_poses_.at(idx_scan + 1);
        pcl::PointCloud<PointType>::Ptr BatchScan = scans2batch(ScanFirst, ScanSecond, PoseFirst, PoseSecond);

        ROS_INFO_STREAM("\033[1;32m -- ... have been solved" << (idx_scan) << " number" "\033[0m");
        // scan's pointcloud to range img ，生成深度图，_scan是在激光雷达坐标系下的
        cv::Mat scan_rimg = scan2RangeImg(BatchScan, kFOV, _rimg_shape, idx_scan); // openMP inside
        RemoveFormMatScansSavePNG(scan_rimg, kRangeColorAxis, idx_scan);
        // map's pointcloud to range img
        if (kUseSubsetMapCloud) {
            takeGlobalMapSubsetWithinBall(idx_scan);
        ...
        publishPointcloud2FromPCLptr(scan_publisher_, BatchScan);
    } // for_each scan Done
...
/// _num_all_points是动态点云的数量
std::vector<int>
Removerter::getStaticIdxFromDynamicIdx(const std::vector<int> &_dynamic_point_indexes, int _num_all_points) {
...
std::vector<int> Removerter::getGlobalMapStaticIdxFromDynamicIdx(pcl::PointCloud<PointType>::Ptr input,
                                                                 const std::vector<int> &_dynamic_point_indexes) {
    int num_all_points = input->points.size();
...
void Removerter::removeOnce(float _res_alpha) {
    ...
    float deg_per_pixel = 1.0 / _res_alpha; /// 分辨率的倒数是每个像素之间的间隔，即矩阵的每个元素之间差deg_per_pixel度，360/deg_per_pixel也可以得到矩阵的维度
    ROS_INFO_STREAM("\033[1;32m Removing starts with resolution: x" << _res_alpha << " (" << deg_per_pixel
                                                                    << " deg/pixel)\033[0m");
    ROS_INFO_STREAM(
            "\033[1;32m -- The range image size is: [" << rimg_shape.first << ", " << rimg_shape.second << "].\033[0m");
//    ROS_INFO_STREAM("\033[1;32m -- The number of map points: " << map_global_curr_->points.size() << "\033[0m");
    ...
    /// remove for each scan
//    std::vector<int> dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdxForEachScan( rimg_shape );
    /// remove for batch scan
    std::vector<int> dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdxForBatchScan(rimg_shape);

    ROS_INFO_STREAM("\033[1;32m -- The number of dynamic points: " << dynamic_point_indexes.size() << "\033[0m");
    /// 获得动态点的地图
    parseDynamicMapPointcloudUsingPtIdx(map_global_curr_, dynamic_point_indexes, map_global_curr_dynamic_);

    // static_point_indexes == complemently indexing dynamic_point_indexes
    /// 获得静态点的索引
    std::vector<int> static_point_indexes = getGlobalMapStaticIdxFromDynamicIdx(map_global_curr_,
                                                                                dynamic_point_indexes);
    ROS_INFO_STREAM("\033[1;32m -- The number of static points: " << static_point_indexes.size() << "\033[0m");
    /// 获得静态点地图
    parseStaticMapPointcloudUsingPtIdx(map_global_curr_, static_point_indexes, map_global_curr_static_);
    ROS_INFO_STREAM("\033[1;32m -- get static map " "\033[0m");
    // Update the current map and reset the tree

    map_global_curr_->clear();
    /// 静态地图设置为全局地图，全地图中的动态点云取出后就是静态点云，map_global_curr_static_，相当于每次分辨率都会过滤掉一部分，下次的过滤都会在上一次的分辨率产生的静态地图上产生
    *map_global_curr_ = *map_global_curr_static_;
    *map_global_curr_dynamic_all_res += *map_global_curr_dynamic_;
    ROS_INFO_STREAM("\033[1;32m -- the end of one remove " "\033[0m");
    // if(kUseSubsetMapCloud) // NOT recommend to use for under 5 million points map input
    //     kdtree_map_global_curr_->setInputCloud(map_global_curr_); 
...
void Removerter::revertOnce(float _res_alpha) {
    ...
    ROS_INFO_STREAM(
            "\033[1;32m -- The range image size is: [" << rimg_shape.first << ", " << rimg_shape.second << "].\033[0m");
    ROS_INFO_STREAM(
            "\033[1;32m -- The number of map points: " << map_global_curr_dynamic_all_res->points.size() << "\033[0m");
    /// revert 的主函数
    std::vector<int> dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdxForEachScanForRevert(rimg_shape);
    ROS_INFO_STREAM("\033[1;32m -- The number of dynamic points: " << dynamic_point_indexes.size() << "\033[0m");
    /// 获得动态点的地图
    parseDynamicMapPointcloudUsingPtIdx(map_global_curr_dynamic_all_res, dynamic_point_indexes,
                                        map_global_curr_dynamic_);

    std::vector<int> static_point_indexes = getGlobalMapStaticIdxFromDynamicIdx(map_global_curr_dynamic_all_res,
                                                                                dynamic_point_indexes);
    ROS_INFO_STREAM("\033[1;32m -- The number of static points: " << static_point_indexes.size() << "\033[0m");
    /// 获得静态点地图
    parseStaticMapPointcloudUsingPtIdx(map_global_curr_dynamic_all_res, static_point_indexes, map_global_curr_static_);
///// 需要把所有的静态地图都加起来
//    map_global_curr_->clear();
    *map_global_curr_ += *map_global_curr_static_;
    map_global_curr_dynamic_all_res->clear();
...
void Removerter::revertOnceForOurs() {
    kdtree_dyna_global_curr_->setInputCloud(map_global_curr_dynamic_all_res);

    // for each scan
    for (std::size_t idx_scan = 0; idx_scan < scans_.size(); idx_scan++) {
    revertDynamicPointsByKnn(idx_scan);
    }
}
...
pcl::PointCloud<PointType>::Ptr
Removerter::local2global(const pcl::PointCloud<PointType>::Ptr &_scan_local, int _scan_idx) {
...
pcl::PointCloud<PointType>::Ptr
Removerter::global2local(const pcl::PointCloud<PointType>::Ptr &_scan_global, int _scan_idx) {
...
Removerter::removeDynamicPointsOfScanByKnn(int _scan_idx) {
    ...
    for (std::size_t pt_idx = 0; pt_idx < num_points_of_a_scan; pt_idx++) {
        ...
        ///kNumKnnPointsToCompare is the number of point to search for
        ...
        /// add all the number in topk_L2dists_scan
        ...
        kdtree_map_global_curr_->nearestKSearch(scan_orig_global->points[pt_idx], kNumKnnPointsToCompare,
    ...
    ROS_INFO_STREAM(
            "\033[1;32m -- The number of static points in a scan: " << scan_static_local->points.size() << "\033[0m");
...
void Removerter::revertDynamicPointsByKnn(int _scan_idx) {
    pcl::PointCloud<PointType>::Ptr scan_orig = scans_.at(_scan_idx);
    auto scan_pose = scan_poses_.at(_scan_idx);
    ...
    pcl::PointCloud<PointType>::Ptr scan_dynamic_global(new pcl::PointCloud<PointType>);
    for (std::size_t pt_idx = 0; pt_idx < num_points_of_a_scan; pt_idx++) {
            /// if the points in scan are in map_global_curr_dynamic_all_res too
//        if (std::find(map_global_curr_dynamic_all_res->points.begin(), map_global_curr_dynamic_all_res->points.end(),
//                      scan_orig_global->points[pt_idx]) != map_global_curr_dynamic_all_res->points.end()) {

            std::vector<int> topk_indexes_scan;
            std::vector<float> topk_L2dists_scan;
            ///kNumKnnPointsToCompare is the number of point to search for
            kdtree_dyna_global_curr_->nearestKSearch(scan_orig_global->points[pt_idx], kNumKnnPointsToCompare,
                                                     topk_indexes_scan, topk_L2dists_scan);
            /// add all the number in topk_L2dists_scan
            float sum_topknn_dists_in_scan = accumulate(topk_L2dists_scan.begin(), topk_L2dists_scan.end(), 0.0);
            float avg_topknn_dists_in_scan = sum_topknn_dists_in_scan / float(kNumKnnPointsToCompare);
            ...
            //
            if (std::abs(avg_topknn_dists_in_scan - avg_topknn_dists_in_map) < kScanKnnAndMapKnnAvgDiffThreshold) {
                map_global_static->push_back(map_global_curr_dynamic_all_res->points[pt_idx]);
            }
//        }
    }
}
...
void Removerter::scansideRemovalForEachScanAndSaveThem(void) {
    /// knn search for one scan removal
    scansideRemovalForEachScan();
    saveCleanedScans();
    saveMapPointcloudByMergingCleanedScans();
...
void Removerter::run(void) {
    // load scan and poses,Delete some non-compliant PCDs, leaving valid poses and pCDs
    parseValidScanInfo();

    // Downsampling, save the processed result
    readValidScans();

    // construct initial map using the scans and the corresponding poses
    makeGlobalMap();

    // RunLidar to BEV And Remove
    CloudFilter FilterBEV(this);
    ROS_INFO_STREAM("\033[1;32m -- start removeFloor ********" "\033[0m");
    scans_ = FilterBEV.RunLidar2BEVAndRemove();
    ROS_INFO_STREAM("\033[1;32m --Finish**************"  << "\033[0m");

    /// RANSAC地面点分割
//    RANSAC_seg *ransacSeg = new RANSAC_seg(this);
//    ransacSeg->RANSAC_segmentation();

    // map-side removals
    for (float _rm_res: remove_resolution_list_) {
        removeOnce(_rm_res);
    }
    /// after all the remove_resolution_list_, Save the strictest static point(map_global_static include the ground points)
    *map_global_static += *map_global_curr_;
    ...
    saveCurrentStaticAndDynamicPointCloudLocal(
            base_node_idx_); // w.r.t specific node's coord. 0 means w.r.t the start node, as an Identity.
    ROS_INFO_STREAM("\033[1;32m -- start revert********" "\033[0m");
...
//    revertOnceForOurs();
    *map_global_static += *map_global_curr_;
    ...
    ROS_INFO_STREAM("\033[1;32m -- revert end ******" "\033[0m");
    saveFinalStaticAndDynamicPointCloudGlobal();
    ROS_INFO_STREAM("\033[1;32m -- save the final map**********" "\033[0m");
